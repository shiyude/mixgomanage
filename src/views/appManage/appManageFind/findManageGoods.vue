<template>
  <div class="find_manage_goods">
    <h1 class="hint_site">
      {{$t('current_position')}}： {{$t('app_manage')}} > {{$t('find')}} > {{$t('manage_goods')}}
    </h1>
    <div class="FindCreation_form">
      <div class="manufacturers_head clearfix">
        ...
        <div class="right">
          ...
          <el-button icon="el-icon-back" @click="$router.replace('appManageFind.html')">{{$t("go_back")}}</el-button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      interfaceConfig: {
        api: this.$api.queryDiscoveryGoods,
        params: {
          discoveryId: this.$route.query.id,
          action: "add"
        },
        keyWord: "goodsName"
      }
  ...
  methods: {
    ...
    getDataList(page = 1) {
      this.$Posting(this.$api.queryDiscoveryGoods, {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm(this.$t("alert_delete"), this.$t("alert_command"), {
        ...
        this.$Posting(this.$api.deleteDiscoveryGoods, {
          goodsId: id,
          discoveryId: this.$route.query.id
        }).then(res => {
          if (res.code == 0) {
    ...
    submitData(jsonData) {
      let _this = this;
      this.$Posting(this.$api.saveDiscoveryGoods, jsonData, true).then(res => {
        if (res.code == 0) {
