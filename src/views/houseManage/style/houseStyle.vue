<template>
  <div class="house_style_group">
    <h1 class="hint_site">
      <!-- 当前位置：户型管理 > 风格列表 -->
      {{$t('current_position')}}： {{$t('house_type_manage')}} > {{$t('style_list')}}
    </h1>
    <div class="manufacturers_head clearfix">
      ...
      <div class="right">
        <el-button @click="addData" type="primary" icon="el-icon-circle-plus">{{$t('add_style')}}</el-button>
      </div>
    </div>
    <!-- 表格 -->
    <div class="manufacturers_tab">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column :label="$t('name2')" v-if="langType == 2 || langType == 3">
          <template slot-scope="scope">
            <span>{{ scope.row.styleNameEn }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="$t('set_style')" :visible.sync="dialogVisible" :before-close="closeDialog" width="30%" center>
      <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0">
        <el-form-item label="" prop="name" :rules="{
            required: true,
            message: $t('verificate_style_name1'),
            trigger: 'blur'
          }" v-if="langType == 1 || langType == 3">
          <el-input v-model.trim="ruleForm.name" :placeholder="$t('style_name1')"></el-input>
        </el-form-item>
      <el-form-item label="" prop="name1" :rules="{
            required: true,
            message: $t('verificate_style_name2'),
            trigger: 'blur'
          }" v-if="langType == 2 || langType == 3">
        <el-input v-model.trim="ruleForm.name1" :placeholder="$t('style_name2')"></el-input>
      </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm(this.$t("alert_delete"), this.$t("alert_command"), {
        ...
        this.$Posting(this.$api.deleteHouseStyle, {
          styleId: styleId
        }).then(res => {
          if (res.code == 0) {
            this.$notify.success({
              ttitle: this.$t("alert_success_title"),
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let dataObj = {
            styleName: _this.ruleForm.name,
            styleNameEn: _this.ruleForm.name1
          };
          if (this.selectData === null) {
            this.submitData(dataObj, 0);
          } else {
            dataObj.styleId = this.selectData.styleId;
    ...
    //表单渲染的方法
    renderForm(data) {
      this.ruleForm.name = data.styleName;
      this.ruleForm.name1 = data.styleNameEn;
    },
    // 重置表单
    resetForm() {
    ...
    submitData(jsonData, type = 0) {
      ...
      this.$Posting(this.$api.saveHouseStyle, jsonData).then(res => {
        if (res.code == 0) {
          _this.resetForm();
