<template>
  <div class="add_brand">
    <h1 class="hint_site">
      <!-- 当前位置：品牌管理 > 添加分组品牌 -->
      {{$t('current_position')}}： {{$t('brand_manage')}} > {{$t('add_brand_group')}}
    </h1>
    <div class="FindCreation_form">
      <div class="manufacturers_head clearfix">
        ...
        <div class="right">
          ...
          <el-button icon="el-icon-back" @click="$router.replace('brandGroup.html')">{{$t('go_back')}}</el-button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getDataList(page = 1) {
      this.$Posting(this.$api.queryBrandByGroupId, {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm(this.$t("alert_delete"), this.$t("alert_command"), {
        ...
        this.$Posting(this.$api.deleteBrandGroupJoinByBrandId, {
          brandId: id,
          groupId: this.$route.query.id
        }).then(res => {
          if (res.code == 0) {
    ...
    submitData(jsonData) {
      let _this = this;
      this.$Posting(this.$api.saveBrandGroupJoin, jsonData, true).then(res => {
        if (res.code == 0) {
